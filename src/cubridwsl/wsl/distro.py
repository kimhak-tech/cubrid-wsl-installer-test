...
def exists(name: str | None) -> bool:
    """Is this distribution registered with WSL?

    Takes the name rather than looking it up, so it still answers after the
    registry key that named it has been removed -- which is exactly when a
    removal case asks. A missing name is False, not an error: there is nothing
    to find.

    Never raises. `wsl -l -v` failing is reported as "not there" only when the
    list really came back without it; a failure to list at all propagates as a
    DistroError from `list_distros`.
    """
    if not name:
        return False
